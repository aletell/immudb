...
// SetBatchHandler ...
type SetBatchHandler interface {
	SetBatch(w http.ResponseWriter, req *http.Request, pathParams map[string]string)
}

type setBatchHandler struct {
	mux    *runtime.ServeMux
	client client.ImmuClient
}

// NewSetBatchHandler ...
func NewSetBatchHandler(mux *runtime.ServeMux, client client.ImmuClient) SetBatchHandler {
	return &setBatchHandler{
		mux:    mux,
		client: client,
	}
}

func (h *setBatchHandler) SetBatch(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
	...
	batchRequest, err := new(client.BatchRequest).FromKVList(&protoReq, prefix)
	if err != nil {
		runtime.HTTPError(ctx, h.mux, outboundMarshaler, w, req, err)
		return
	}

	msg, err := h.client.SetBatch(rctx, batchRequest)
	if err != nil {
		runtime.HTTPError(ctx, h.mux, outboundMarshaler, w, req, err)
		return
	}
