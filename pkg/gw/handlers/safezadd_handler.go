...
func (h *safeZAddHandler) SafeZAdd(w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
	...
	key, _, err := prefixKey(req, protoReq.GetZopts().GetKey())
	if err != nil {
		runtime.HTTPError(ctx, h.mux, outboundMarshaler, w, req, err)
		return
	}
	set, _, err := prefixKey(req, protoReq.GetZopts().GetSet())
	if err != nil {
		runtime.HTTPError(ctx, h.mux, outboundMarshaler, w, req, err)
		return
	}

	msg, err := h.client.SafeZAdd(rctx, set, protoReq.Zopts.Score, key)
	if err != nil {
		runtime.HTTPError(ctx, h.mux, outboundMarshaler, w, req, err)
		return
	}
